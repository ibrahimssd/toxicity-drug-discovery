...
class BPELearner():
    # this part applies only for smiles
    ...
    def learn_BPE(self):
        # this part applies for both smiles and Selfies
        ...
        # Set up logging
        log_file = "./logs/bpe_training.log"
        logging.basicConfig(filename=log_file, level=logging.INFO,
                            format="%(asctime)s [%(levelname)s] %(message)s", datefmt="%Y-%m-%d %H:%M:%S")

        logging.info("Training BPE with %d tokens", voc_size)
        logging.info("Input file: %s", file_name)
        logging.info("min_frequency: %d", self.min_frequency)
