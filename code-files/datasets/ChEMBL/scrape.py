...
import os
import sys
# github link for the code: https://github.com/IoDiakou/Automatic-collection-of-drugs-for-specific-target-from-ChEMBL
...
output_stats_file = 'stats.txt'
output_stats_file_path = os.path.join(output_dir, output_stats_file)

# Open the output txt file in write mode
with open(output_stats_file_path, 'w') as txtfile:
    txtfile.write(f"ChEMBL ID\tOutput File\tNumber of Retrieved SMILES\n")
    # Write the statistics to the text file
    txtfile.write(f"{chembl_id}\t{output_file}\t{len(all_data)}\n")
